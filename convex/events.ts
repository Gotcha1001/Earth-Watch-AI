      ...
      try {
        ...
        if (existingByExternalId) {
          await ctx.db.patch(existingByExternalId._id, {
            ...
            description: event.description,
            ...(event.locationName
              ? { locationName: event.locationName, needsGeocode: false } // NEW
              : {}),
            sourceUrl: event.sourceUrl,
            status: "active",
            ingestedAt: now,
        ...
        await ctx.db.insert("disasterEvents", {
          ...
          ingestedAt: now,
          status: "active",
          needsGeocode: !event.locationName, // NEW — flagged at birth
        });
        inserted += 1;
      } catch (error) {
        failures += 1;
...
const VOLCANO_LIVE_MAP_WINDOW_MS = 1000 * 60 * 60 * 48;
export const listActiveEvents = query({
  args: {},
  handler: async (ctx) => {
    const cutoff = Date.now() - VOLCANO_LIVE_MAP_WINDOW_MS;
...
export const patchLocationName = internalMutation({
  args: { eventId: v.id("disasterEvents"), locationName: v.string() },
  handler: async (ctx, { eventId, locationName }) => {
    await ctx.db.patch(eventId, { locationName, needsGeocode: false }); // CHANGED
  },
});

// CHANGED — was an unindexed `.filter()` full-table scan, run every single
// minute regardless of whether anything actually needed geocoding. Now
// reads the `by_needsGeocode` index directly, so an empty/quiet tick
// resolves almost immediately instead of scanning every row.
export const listEventsMissingLocationName = internalQuery({
  args: { limit: v.number() },
  handler: async (ctx, { limit }) => {
    return ctx.db
      .query("disasterEvents")
      .withIndex("by_needsGeocode", (q) => q.eq("needsGeocode", true))
      .take(limit);
  },
});
...
const VOLCANO_RECENTLY_CONFIRMED_MS = 1000 * 60 * 30;
export const listErupting = query({
  args: {},
  handler: async (ctx) => {
    const cutoff = Date.now() - VOLCANO_RECENTLY_CONFIRMED_MS;
